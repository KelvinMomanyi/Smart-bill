...
| Image OCR                        | Pass locally / production credentials recommended | The real Tesseract runtime read a generated PNG invoice and the parser recovered invoice `SMOKE-1001` with total USD 58.00. Vercel uses Google Cloud Vision when `GOOGLE_APPLICATION_CREDENTIALS_JSON` is configured and falls back to bounded Tesseract processing when Google is unavailable.                                                                                                                                 |

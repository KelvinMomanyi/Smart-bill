...
1. Upload a PDF/image or paste invoice text. Dashboard uploads use the original browser Tesseract processor with live progress. Originals are stored privately before recognition and saved results go to invoice review. Identical uploads and repeated supplier invoice numbers cannot create a second invoice.
...
Dashboard capture and Retry/Continue processing use the working OCR flow from `../old/app/components/InvoiceUpload.tsx`: Tesseract.js 4.1.1 in the browser, English recognition with the default settings, original image sizes, and PDF.js 3.11.174 rendering at 2x scale. Each PDF page is recognized in order, with page markers, progress, an image preview and raw text. PDFs over the original five-page limit are rejected explicitly rather than silently truncated. The first use downloads the engine and English data; subsequent recognition uses the browser cache. Keep the page open until saving completes. Google credentials and `OCR_TIMEOUT_MS` do not affect this browser processor.

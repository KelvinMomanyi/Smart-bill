...
OCR uses Google Cloud Vision first on Vercel when credentials are available. Set `GOOGLE_APPLICATION_CREDENTIALS_JSON` to the full service-account JSON and enable the Cloud Vision API in that Google Cloud project. On a persistent local host, `GOOGLE_APPLICATION_CREDENTIALS` can point to the credential JSON file. `OCR_PROVIDER=auto` gives Google four seconds per request before falling back to the bundled Tesseract engine, leaving most of the bounded processing window for local OCR. `OCR_PROVIDER=google` makes Google mandatory, while `OCR_PROVIDER=tesseract` explicitly selects the local engine. Small images are resized to a practical OCR resolution, very large images are capped, and text-based PDF pages bypass image OCR.

...
OCR defaults to Google Cloud Vision on Vercel. Set `GOOGLE_APPLICATION_CREDENTIALS_JSON` to the full service-account JSON and enable the Cloud Vision API in that Google Cloud project. On a persistent local host, `GOOGLE_APPLICATION_CREDENTIALS` can point to the credential JSON file. `OCR_PROVIDER=auto` uses Google when valid credentials are available and otherwise uses the bundled Tesseract engine. `OCR_PROVIDER=tesseract` explicitly selects the local engine. Text-based PDF pages bypass image OCR.
